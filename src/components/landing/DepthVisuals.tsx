...
export function ParticleLayer({ count, mobileCount = 8, tone = "digital" }: ParticleLayerProps) {
  const colors = {
    ...
    digital: "bg-cyan-200 shadow-[0_0_9px_rgb(34_211_238_/_0.42)]",
    core: "bg-violet-100 shadow-[0_0_11px_rgb(139_92_246_/_0.58)]",
  };
    ...
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
      {Array.from({ length: count }, (_, index) => (
        <span
          key={index}
          data-depth-particle
          className={`absolute rounded-full ${colors[tone]} ${index >= mobileCount ? "hidden md:block" : ""}`}
          style={{
            left: `${(index * 37 + 11) % 96}%`,
            top: `${(index * 53 + 7) % 92}%`,
            width: `${1 + (index % 3)}px`,
            height: `${1 + (index % 3)}px`,
            opacity: 0.16 + (index % 5) * 0.09,
          }}
        />
      ))}
    </div>
...
export function ServiceCard({ service, index }: { service: JourneyService; index: number }) {
  return (
    <article
      data-service-card
      className="group relative min-h-44 overflow-hidden rounded-[1.4rem] border border-white/10 bg-[linear-gradient(145deg,rgb(17_20_31_/_0.88),rgb(8_10_18_/_0.72))] p-6 shadow-[inset_0_1px_rgb(255_255_255_/_0.05),0_20px_70px_rgb(0_0_0_/_0.3)] backdrop-blur-xl"
    >
      <div className="absolute inset-x-7 top-0 h-px bg-gradient-to-r from-transparent via-violet-300/55 to-transparent" />
      <div className="mb-7 flex items-center justify-between gap-4">
        ...
        <span className="h-px flex-1 bg-gradient-to-r from-violet-300/20 to-transparent" />
      </div>
...
export function WorkflowVisualization({
  ...
  return (
    <div
      ...
      className="relative rounded-[2rem] border border-cyan-200/10 bg-[#070a16]/70 p-5 shadow-[inset_0_0_50px_rgb(34_211_238_/_0.035),0_30px_100px_rgb(0_0_0_/_0.42)] backdrop-blur-xl md:p-7"
    >
      <div
        aria-hidden="true"
      ...
      >
        {labels.map((label, index) => (
          <div key={label} className="contents">
            ...
            >
              <span className="absolute left-3 top-3 size-1.5 rounded-full bg-cyan-300 shadow-[0_0_10px_rgb(34_211_238_/_0.85)]" />
              {label}
            </div>
            {index < labels.length - 1 ? (
              ...
              >
                <span className="absolute inset-0 origin-top bg-gradient-to-b from-cyan-300 via-violet-400 to-transparent shadow-[0_0_10px_rgb(34_211_238_/_0.65)] md:origin-left md:bg-gradient-to-r" />
              </div>
            ) : null}
          </div>
        ))}
...
export function NeuralCoreVisualization() {
  return (
    <div
      role="img"
      aria-label="Abstract artificial intelligence network"
    ...
    >
      ...
      />
      <div className="absolute inset-[18%] rounded-full bg-[radial-gradient(circle,rgb(34_211_238_/_0.18),rgb(124_58_237_/_0.16)_40%,transparent_72%)] blur-xl" />
      ...
      {neuralNodes.map((node, index) => (
        <span
          key={index}
          data-neural-node
          className={`absolute rounded-full border border-white/20 bg-cyan-200 shadow-[0_0_13px_rgb(34_211_238_/_0.7)] ${index >= 9 ? "hidden md:block" : ""}`}
          style={{
            left: `${node.left}%`,
            top: `${node.top}%`,
            width: node.size,
