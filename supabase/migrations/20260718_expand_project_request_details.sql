-- Persist the complete Start Project wizard request without changing existing IDs, prices, or rows.
-- This migration is safe to run after 20260717_fix_project_request_submission.sql.
...
alter table public.project_requests
  drop constraint if exists project_requests_language_count_positive;
alter table public.project_requests
  add constraint project_requests_language_count_positive
  check (language_count >= 1);

-- Replace the older 17-argument RPC so PostgREST exposes only one unambiguous function.
  ...
  for attempt in 1..5 loop
    ...
    begin
      insert into public.project_requests (
        ...
        timeline_option, requested_date, is_rush, contact_method, notes,
        ...
        clean_project_type, clean_project_idea, clean_package_id, p_included_features,
        p_selected_features, clean_custom_features, coalesce(p_language_count, 1),
        clean_timeline_option, p_requested_date, coalesce(p_is_rush, false), clean_contact_method,
        clean_notes, p_estimated_min_jod, p_estimated_max_jod, clean_currency, p_converted_min,
